...
let OTelService = class OTelService {
    ...
    get baseUrl() {
        ...
        if (!url)
            throw new Error("Missing TEMPO_URL environment variable");
        return url;
    }
    get authHeaders() {
    ...
    async fetchTraceSpans(traceId, filterStatus) {
        try {
            ...
            let spans = data?.batches?.flatMap((b) => b.instrumentationLibrarySpans?.flatMap((ils) => ils.spans)) || [];
            if (filterStatus === 'ERROR') {
                spans = spans.filter((s) => s.status?.code === 'STATUS_CODE_ERROR');
            }
    ...
    async fetchServiceDependencyGraph(traceId) {
        try {
            const response = await fetch(`${this.baseUrl}/api/traces/${traceId}`, {
                headers: this.authHeaders
            });
            if (!response.ok)
                throw new Error(`HTTP error! status: ${response.status}`);
            const data = await response.json();
            const spans = data?.batches?.flatMap((b) => b.instrumentationLibrarySpans?.flatMap((ils) => ils.spans)) || [];
            const graph = { edges: spans.map((s) => ({ from: s.parentSpanId || 'root', to: s.spanId, name: s.name })) };
    ...
    async fetchTraceWaterfall(traceId) {
        try {
            const response = await fetch(`${this.baseUrl}/api/traces/${traceId}`, {
                headers: this.authHeaders
            });
    ...
    parseLocalOtelLog(logFilePath, traceId) {
        ...
        for (const line of lines) {
            try {
                ...
                if (line.trimStart().startsWith('{')) {
                    ...
                    if (!id.includes(traceId.replace(/-/g, '').toLowerCase()))
                        continue;
                    spans.push({
                        traceId: id,
                ...
                if (!id || !id.includes(traceId.replace(/-/g, '').toLowerCase()))
                    continue;
                spans.push({
                    traceId: id,
        ...
        const errorSpans = spans.filter(s => s.status === 'STATUS_CODE_ERROR' || s.status === 'ERROR' || s.status === '2');
        const lines2 = [
            `## OTel Trace Summary`,
            `Trace ID : ${traceId}`,
        ...
        (errorSpans.length ? errorSpans : spans).slice(0, 20).forEach(s => {
            lines2.push(`- [${s.serviceName}] ${s.name} | spanId=${s.spanId} parentId=${s.parentId || 'root'} ` +
     ...
     */
    _parseDockerComposeLog(lines, traceId) {
        ...
        const matched = records.filter(r => r.some(l => {
        ...
        const errors = spans.filter(s => s.severity === 'error' || s.severity === 'ERROR' || s.severity === 'warn');
        const out = [
