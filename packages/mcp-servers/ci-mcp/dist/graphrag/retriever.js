...
export class CISubgraphRetriever {
    ...
    forRun(raw) {
        ...
        for (const jobId of failedJobIds) {
            for (const { target: stepId, attrs } of G.edgesFrom(jobId)) {
                ...
                if (conclusion !== 'failure' && exitCode === 0)
                    continue;
                failedSteps.push({
                    id: stepId,
                ...
                for (const { target: errId, attrs: errAttrs } of G.edgesFrom(stepId)) {
                    if (errAttrs['relation'] !== 'EMITS_ERROR')
                        continue;
                    if (seenErrors.has(errId))
                        continue;
        ...
        for (const { target: fileId, attrs } of G.edgesFrom(runNodeId)) {
            if (attrs['relation'] !== 'TOUCHES_FILE')
                continue;
            const fileNode = G.getNode(fileId);
            if (fileNode)
                changedFiles.push(String(fileNode['path'] ?? fileId.replace('file:', '')));
        }
     ...
     */
    suggestFiles(errors, changedFiles) {
        if (!errors.length || !changedFiles.length)
 ...
 */
function extractModuleTokens(message) {
    const tokens = [];
 ...
 */
function fileMatchesToken(filePath, token) {
...
function buildContextText(ctx) {
    ...
    lines.push(`## Run ${ctx.runMeta.run_id} — ${ctx.runMeta.conclusion.toUpperCase()} ` +
    ...
    if (ctx.suggestedFiles.length) {
        ...
        for (const f of ctx.suggestedFiles)
            lines.push(`  - ${f}`);
    }
    return lines.join('\n');
}
