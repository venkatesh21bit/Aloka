...
function extractErrors(logTail) {
    const errors = [];
    const seen = new Set();
    ...
    while ((m = errorLineRe.exec(logTail)) !== null) {
        ...
        if (errors.length >= 5)
            break; // at most 5 errors per step
    }
    return errors;
}
/** Stable hash for deduplicating error nodes across steps. */
function errorHash(message) {
    let h = 0;
...
export class CIPipelineGraph {
    ...
    toDict() {
        ...
        const edges = this.graph.nodesWithData().flatMap(([source]) => this.graph.edgesFrom(source).map(({ target, attrs }) => ({ source, target, ...attrs })));
        return { nodes, edges };
    }
    ...
    failedStepIds() {
        return this.graph.nodesByType('step').filter(id => {
            ...
            return node?.['conclusion'] === 'failure' || node?.['exit_code'] !== 0;
        });
    }
    changedFileIds() {
